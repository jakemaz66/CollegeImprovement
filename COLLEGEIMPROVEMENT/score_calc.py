...
def plots():
    ...
    data_group = df.groupby('University')['Score'].mean().reset_index()
    duquesne_df = df_grouped.loc[df_grouped['University'] == 'Duquesne University', 'Score'].values[0]
    score_rank = (data_group['Score'] > duquesne_df).sum() + 1  
    print(f"Duquesne University has a mean 'Score' rank of: {score_rank} out of {len(data_group)} Universities.")

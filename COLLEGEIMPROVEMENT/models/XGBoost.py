...
predictions = xgb.predict(df_model_imputed[['ADM_RATE', 'TUITIONFEE_IN', 'ADMCON7', 'AVGFACSAL', 'INEXPFTE', 'STUFACR', 'PRGMOFR', 'UGDS']])
df_final['Pred_COUNT_WNE_P10'] = predictions

xgb.fit(scaled_x, y_train2)
predictions = xgb.predict(df_model_imputed[['ADM_RATE', 'TUITIONFEE_IN', 'ADMCON7', 'AVGFACSAL', 'INEXPFTE', 'STUFACR', 'PRGMOFR', 'UGDS']])
df_final['Pred_MD_EARN_WNE_P10'] = predictions

xgb.fit(scaled_x, y_train3)
predictions = xgb.predict(df_model_imputed[['ADM_RATE', 'TUITIONFEE_IN', 'ADMCON7', 'AVGFACSAL', 'INEXPFTE', 'STUFACR', 'PRGMOFR', 'UGDS']])

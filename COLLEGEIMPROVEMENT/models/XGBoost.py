...
df_final = pd.DataFrame(columns=['University', 'Predicted Salary', 'Predicted Debt', 'Predicted Job',
                                 ...
                                 'Undergraduate Enrollment', 'Full-Time Faculty'])
...
df_final['Full-Time Faculty'] = df_model_imputed['PFTFAC']

#Splitting the data
X = df_model_imputed[['ADM_RATE', 'TUITIONFEE_IN', 'ADMCON7',
         'AVGFACSAL', 'INEXPFTE', 
        'STUFACR', 'PRGMOFR', 'UGDS', 'PFTFAC']]
...
xgb = XGBRegressor(learning_rate= 0.2, max_depth= 7, n_estimators= 200)

#Predicting Job Rate
xgb.fit(scaled_x, y_train)

predictions = xgb.predict(scaler.transform(df_model_imputed[['ADM_RATE', 'TUITIONFEE_IN', 'ADMCON7', 'AVGFACSAL', 'INEXPFTE', 'STUFACR', 'PRGMOFR', 'UGDS', 'PFTFAC']]))
